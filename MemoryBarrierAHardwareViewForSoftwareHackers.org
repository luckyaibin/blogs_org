...
* 3 不必要的延迟情况下存储结果
  如果cpu0想要写（write）一个在cpu0 的 cache 里的某个cache line，cpu0在能够写之前要等待缓存行到达cache0(cpu0 invalidate 给cpu1，cpu1 发送acknowledgement给cpu0,)。
  
** 3.1 Store Buffers
  为了速度，在cpu 和 cache 之间增加了storebuffer。cpu0 能够简单的把写操作存在storebuffer里，然后继续执行。等到最终cache line从cpu1 到达 cpu0 的时候，数据就从store buffer写到cpu0的cache line。
